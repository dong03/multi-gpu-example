...
def read_annotations_depth(data_path):
    depth_dir = "/data_activate/chenxinru/dataset/ps-battles/megadepth"

    lines = map(str.strip, open(data_path).readlines())
    data = []
    for line in lines:
        sample_path, mask_path, label = line.split()
        name = os.path.split(sample_path)[-1]
        depth_path = os.path.join(depth_dir, data_path.split("/")[-1][:-4], name)
        # depth_path = os.path.join('/data_activate/dongchengbo/dataset/ps_battles_orisize/depth', name)
        label = int(label)
        data.append((sample_path, mask_path, depth_path, label))
    return data
...
#     data = []
#     for line in lines:
#         sample_path, mask_path, depth_path, label = line.split()
#         label = int(label)
#         data.append((sample_path, mask_path, depth_path, label))
#     return data
