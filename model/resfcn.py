...
class ResFCN(ResNet50):
    ...
    def forward(self, x):
        ...
        temp1 = torch.mean(c4,dim=1).reshape(-1)
        temp2 = torch.mean(c4.reshape(4096,-1),dim=1)
        # return torch.cat([temp1,temp2]), torch.cat([temp1,temp2]), torch.cat([temp1,temp2])
        outputs = []
        ...
        score = self.fc(torch.flatten(x0,1))
        return score
